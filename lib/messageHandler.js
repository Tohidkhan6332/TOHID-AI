...
const { handleAgentResponse } = require('./tohidAgent');
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, { 
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029VaGyP933bbVC7G0x0i2T](https://chat.whatsapp.com/HUEyTVIQ7Ij1gFs6aZkbMk)' 
                }, { quoted: message });
                return;
        ...
        if (!usedPrefix && !command) {
            ...
            const botMode = await store.getBotMode();
            const canUseAgent = botMode === 'public' ||
                                (botMode === 'groups' && isGroup) ||
                                (botMode === 'inbox' && !isGroup) ||
                                isOwnerOrSudoCheck;

            if (canUseAgent && process.env.AI_AGENT_ENABLED !== 'false') {
                const handledByAgent = await handleAgentResponse(sock, chatId, message, {
                    senderId,
                    isGroup,
                    isOwnerOrSudoCheck,
                    senderIsOwnerOrSudo
